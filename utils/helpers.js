const greetingMessage = ({ name }) => {
  ...
  const isPastMidNight = now.getHours() > 0 && now.getHours() <= 3
  const isAlmostMorning = now.getHours() > 3 && now.getHours() <= 5
  ...
  if (isPastMidNight) return `Let's burn some midnight oil, ${name}`
  if (isAlmostMorning) return `Hey there, night owl `
}
